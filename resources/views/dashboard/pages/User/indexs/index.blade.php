...
<table class="table table-striped-custom w-100 mx-auto">
    ...
    <tr>
        ...
        <th scope="col" style="white-space: nowrap;">specialization</th> {{-- إضافة العمود --}}
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                @if($User->image)
                    <img src="{{ asset('storage/profiles/' . $User->image) }}" alt="User Image" width="50" height="50">
                @else
                    <span>No image</span>
                @endif
            </td>
            ...
            <td>
                <form action="{{ route('users.destroy', $User->id) }}" method="POST" class="d-flex justify-content-between align-items-center">
                    @csrf
                    @method('DELETE')
                    <a href="{{ route('users.show', $User->username) }}" class="btn btn-warning btn-sm font-weight-bold fs-6 custom-btn-space">Show</a>

                    @if(auth()->user()->user_type == 'admin')
                        <a href="{{ route('users.edit', $User->id) }}" class="btn btn-success btn-sm font-weight-bold fs-6 custom-btn-space">Edit</a>
        ...
        @empty
        <tr>
            <td colspan="10" class="text-center"> {{-- تحديث العدد ليتوافق مع الأعمدة --}}
